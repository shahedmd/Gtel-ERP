...
// ─────────────────────────────────────────────────────────────
// _ProductDialog — shell (responsive height, not hardcoded 800)
// ─────────────────────────────────────────────────────────────
class _ProductDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Responsive max-height — fills 90% of screen so all form fields reachable
    ...
    return Dialog(
      ...
      child: ConstrainedBox(
        constraints: BoxConstraints(maxWidth: 650, maxHeight: maxH),
        child: Column(
...
// ─────────────────────────────────────────────────────────────
// _SubmitButton — observes isActionLoading, prevents double-tap
// ─────────────────────────────────────────────────────────────
class _SubmitButton extends StatelessWidget {
  final String label;
  final IconData icon;
  final VoidCallback? onPressed; // null = disabled (already loading)
    ...
    return Obx(() {
      ...
      return SizedBox(
        ...
        child: ElevatedButton.icon(
          // Disables taps while loading — prevents double-submission
...
class _EditProductFormState extends State<_EditProductForm> {
  ...
  late final TextEditingController weightC, yuanC, currencyC, shipmentTaxC, shipmentTaxAirC;
  late final TextEditingController airC, seaC, agentC, wholesaleC, shipmentNoC, shipmentDateC, alertQtyC;
  late final TextEditingController stockC, avgPriceC, seaStockC, airStockC, localStockC;
  ...
  void dispose() {
    for (final c in [
      nameC, categoryC, brandC, modelC, weightC, yuanC, currencyC,
      shipmentTaxC, shipmentTaxAirC, airC, seaC, agentC, wholesaleC,
      shipmentNoC, shipmentDateC, alertQtyC, stockC, avgPriceC,
      seaStockC, airStockC, localStockC,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  // ── Submit — guarded against double-tap, only closes on success ─
  Future<void> _submit() async {
    ...
    try {
      ...
      await widget.controller.updateProduct(p.id, {
        ...
        'shipmenttaxair':    double.tryParse(shipmentTaxAirC.text) ?? p.shipmentTaxAir,
        ...
        'avg_purchase_price':double.tryParse(avgPriceC.text) ?? p.avgPurchasePrice,
      ...
      // isActionLoading is reset by controller — only close if loading is done
      // and no error was thrown (controller shows snackbar on error, not exception)
      if (!widget.controller.isActionLoading.value) {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _ResponsiveRow(
          left: _FormField(yuanC, 'Yuan Price (¥)', Icons.money,
              keyboardType: TextInputType.number),
          right: _FormField(weightC, 'Weight (KG)', Icons.monitor_weight,
              keyboardType: TextInputType.number),
        ),
        _ResponsiveRow(
          left: _FormField(shipmentTaxC, 'Sea Tax /KG', Icons.directions_boat,
              keyboardType: TextInputType.number),
          right: _FormField(shipmentTaxAirC, 'Air Tax /KG',
              Icons.airplanemode_active,
              keyboardType: TextInputType.number),
        ),
        _FormField(currencyC, 'Exchange Rate', Icons.currency_exchange,
            keyboardType: TextInputType.number),
        ...
        _ResponsiveRow(
          left: _FormField(shipmentNoC, 'Shipment No', Icons.numbers,
              keyboardType: TextInputType.number),
          right: _FormField(shipmentDateC, 'Shipment Date',
              Icons.calendar_today,
              readOnly: true,
              onTap: () => _pickDate(context, shipmentDateC)),
        ),
        _FormField(alertQtyC, 'Low Stock Alert Qty',
            Icons.notification_important,
            keyboardType: TextInputType.number),

        const _SectionHeader('Sales Pricing'),
        _ResponsiveRow(
          left: _FormField(agentC, 'Agent Price', Icons.person,
              keyboardType: TextInputType.number),
          right: _FormField(wholesaleC, 'Wholesale', Icons.groups,
              keyboardType: TextInputType.number),
        ),

        const _SectionHeader('Stock Override ⚠️'),
        _FormField(avgPriceC, 'Average Purchase Rate (BDT)', Icons.payments,
            keyboardType: TextInputType.number),
        _ResponsiveRow(
          left: _FormField(stockC, 'Total Stock', Icons.inventory_2,
              keyboardType: TextInputType.number),
          right: _FormField(seaStockC, 'Sea Stock', Icons.directions_boat,
              keyboardType: TextInputType.number),
        ),
        _ResponsiveRow(
          left: _FormField(airStockC, 'Air Stock', Icons.airplanemode_active,
              keyboardType: TextInputType.number),
          right: _FormField(localStockC, 'Local Stock', Icons.store,
              keyboardType: TextInputType.number),
        ),
...
class _CreateProductFormState extends State<_CreateProductForm> {
  ...
  late final TextEditingController agentC, wholesaleC, shipmentNoC,
      shipmentDateC, stockC, alertQtyC;
  ...
  void initState() {
    ...
    for (final c in [yuanC, weightC, currencyC, seaTaxC, airTaxC]) {
  ...
  void dispose() {
    for (final c in [
      nameC, categoryC, brandC, modelC, yuanC, weightC, currencyC,
      seaTaxC, airTaxC, airResultC, seaResultC, agentC, wholesaleC,
      shipmentNoC, shipmentDateC, stockC, alertQtyC,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  void _calculate() {
    final yuan   = double.tryParse(yuanC.text) ?? 0.0;
    final weight = double.tryParse(weightC.text) ?? 0.0;
    final curr   = double.tryParse(currencyC.text) ??
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _ResponsiveRow(
          left: _FormField(yuanC, 'Yuan (¥)', Icons.money,
              keyboardType: TextInputType.number),
          right: _FormField(weightC, 'Weight (KG)', Icons.monitor_weight,
              keyboardType: TextInputType.number),
        ),
        _ResponsiveRow(
          left: _FormField(seaTaxC, 'Sea Tax /KG', Icons.directions_boat,
              keyboardType: TextInputType.number),
          right: _FormField(airTaxC, 'Air Tax /KG', Icons.airplanemode_active,
              keyboardType: TextInputType.number),
        ),
        _FormField(currencyC, 'Rate', Icons.currency_exchange,
            keyboardType: TextInputType.number),

        const _SectionHeader('Auto-Calculated Costs'),
        _ResponsiveRow(
          left: _FormField(seaResultC, 'Sea Price (BDT)', Icons.waves,
              readOnly: true),
          right: _FormField(airResultC, 'Air Price (BDT)', Icons.air,
              readOnly: true),
        ),
        ...
        _ResponsiveRow(
          left: _FormField(shipmentNoC, 'Shipment No', Icons.numbers,
              keyboardType: TextInputType.number),
          right: _FormField(shipmentDateC, 'Date', Icons.calendar_today,
              readOnly: true,
              onTap: () => _pickDate(context, shipmentDateC)),
        ),
        _ResponsiveRow(
          left: _FormField(stockC, 'Initial Stock (Sea)', Icons.inventory_2,
              keyboardType: TextInputType.number),
          right: _FormField(alertQtyC, 'Low Stock Alert Qty',
              Icons.notification_important,
              keyboardType: TextInputType.number),
        ),
...
// ─────────────────────────────────────────────────────────────
// _DeleteConfirmDialog — permission check + loading state
// ─────────────────────────────────────────────────────────────
