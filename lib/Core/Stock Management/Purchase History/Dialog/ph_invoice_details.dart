...
// ─────────────────────────────────────────────────────────────────────────────
// INVOICE DETAIL DIALOG  (view + inline edit)
// ─────────────────────────────────────────────────────────────────────────────
...
class _PHInvoiceDetailContentState extends State<_PHInvoiceDetailContent> {
  ...
  void initState() {
    super.initState();
    _editedItems =
        widget.record.items.map((e) => Map<String, dynamic>.from(e)).toList();
    _initControllers();
  }
  ...
  double get _currentTotal {
    ...
    for (int i = 0; i < _editedItems.length; i++) {
      sum +=
          (int.tryParse(_qtyCtrls[i].text) ?? 0) *
          (double.tryParse(_costCtrls[i].text) ?? 0);
    }
    return sum;
  }
  ...
  void _saveChanges() {
    for (int i = 0; i < _editedItems.length; i++) {
      final q = int.tryParse(_qtyCtrls[i].text) ?? 0;
      final c = double.tryParse(_costCtrls[i].text) ?? 0.0;
      _editedItems[i]['qty'] = q;
      _editedItems[i]['cost'] = c;
      _editedItems[i]['subtotal'] = (q * c).toStringAsFixed(2);
    }
    widget.ctrl.editPurchase(
      debtorId: widget.record.debtorId,
      purchaseId: widget.record.id,
      oldItems: List.from(widget.record.items),
...
class _PHItemRow extends StatelessWidget {
  ...
  double get _subtotal {
    ...
    return double.tryParse(item['subtotal'].toString()) ?? 0.0;
  }
          ...
          Expanded(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${item['model'] ?? ''} • ${item['location'] ?? 'Local'}',
                  style: const TextStyle(
                    fontSize: 11,
                    color: PHTokens.slate400,
                  ),
